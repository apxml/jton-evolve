# EVOLVE-BLOCK-START
"""
JSON Compressor v2 - Enhanced Compression

Improvements over v1:
1. More aggressive key abbreviation (threshold reduced to 5)
2. Value compression for repeated strings
3. Boolean compression to 1/0
4. Single-character keys for metadata
"""
...
class JSONCompressor:
    ...
    def _should_compress_string(self, s: str) -> bool:
        """Determine if string should be compressed."""
        # Compress strings longer than 8 characters
        return isinstance(s, str) and len(s) > 8
    
    def _get_short_value(self, value: str) -> Union[str, int]:
        """Get compressed representation of repeated string."""
        if value not in self.value_map:
            short_val = f"v{self.value_counter}"
            self.value_map[value] = short_val
            self.reverse_value_map[short_val] = value
    ...
    def _compress_value(self, value: Any) -> Any:
        ...
        if isinstance(value, dict):
            ...
            # Compress booleans to 1/0
            return 1 if value else 0
        elif isinstance(value, (int, float)):
            return value
        elif isinstance(value, str):
            # Don't compress strings in this version to maintain correctness
            return value
        else:
            return str(value)
    ...
    def compress(self, data: Any) -> Dict[str, Any]:
        ...
        # Build result with single-char keys
    ...
    def _decompress_value(self, value: Any, key_map: Dict[str, str], val_map: Dict[str, str]) -> Any:
        """Recursively decompress a value."""
        if isinstance(value, dict):
            # Check if it's a compressed array
            if "t" in value and value.get("t") == "a":
                return self._decompress_array(value, key_map, val_map)
            else:
                return self._decompress_object(value, key_map, val_map)
        elif isinstance(value, list):
            return [self._decompress_value(item, key_map, val_map) for item in value]
        elif isinstance(value, int) and value in (0, 1):
            # Could be boolean - but how to know?
            # For now, keep as int to avoid ambiguity
            return value
        else:
            return value
    
    def _decompress_array(self, arr_data: Dict, key_map: Dict[str, str], val_map: Dict[str, str]) -> List[Dict]:
        ...
        for i in range(num_rows):
            ...
            for j, short_key in enumerate(short_keys):
                ...
                value = self._decompress_value(columns[j][i], key_map, val_map)
                obj[original_key] = value
            result.append(obj)
        
        return result
    
    def _decompress_object(self, obj: Dict[str, Any], key_map: Dict[str, str], val_map: Dict[str, str]) -> Dict[str, Any]:
        """Decompress a dictionary object."""
        decompressed = {}
        for short_key, value in obj.items():
            original_key = key_map.get(short_key, short_key)
            decompressed[original_key] = self._decompress_value(value, key_map, val_map)
        return decompressed
    ...
    def decompress(self, compressed: Dict[str, Any]) -> Any:
        ...
        val_map = compressed.get("v", {})
        
        # Create reverse mappings
        reverse_key_map = {v: k for k, v in key_map.items()}
        reverse_val_map = {v: k for k, v in val_map.items()}
        
        return self._decompress_value(compressed["d"], reverse_key_map, reverse_val_map)
