# EVOLVE-BLOCK-START
"""
JSON Compressor v32 - Fix type marker collision with strings

Key improvements:
1. Fix bug where strings starting with type markers (U, B, V, H, I, D, T, S) are misidentified
2. Use longer prefixes for packed data to avoid collisions
3. Keep all v26 optimizations
"""
...
class JSONCompressor:
    """String prefix and binary compression with fixed type markers."""
    ...
    def _pack_nums(self, nums: List[Union[int, float]]) -> str:
        """Pack numbers with ~prefix to avoid collisions."""
    ...
    def _d(self, v: Any, km: Dict[str, str]) -> Any:
        ...
        if isinstance(v, dict):
            ...
            # Check for packed data with ~ prefix
            if v[1] == 'S':
    ...
    def _d_arr(self, ad: Dict, km: Dict[str, str]) -> List[Dict]:
        ...
        for col in cols:
            ...
            elif isinstance(col, str) and len(col) > 1 and col[0] == '~':
