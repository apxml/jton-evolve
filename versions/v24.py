# EVOLVE-BLOCK-START
"""
JSON Compressor v24 - Variable-length encoding + dictionary compression

Key improvements:
1. Use varint encoding for small integers
2. Dictionary compression for repeated strings
3. Optimize constant detection threshold
4. Remove unnecessary wrapper objects
"""
...
class JSONCompressor:
    """Variable-length integer and dictionary compression."""
    ...
    def _get_str_ref(self, s: str) -> Union[str, Dict]:
        """Get string reference if beneficial."""
        if len(s) < 10:
            return s
        
        if s not in self.str_dict:
            self.str_dict[s] = self._b62(self.str_counter + 1000)  # Offset to avoid key collision
            self.str_counter += 1
            return s
        else:
            # Return reference
            return {"r": self.str_dict[s]}
    
    def _b62(self, num: int) -> str:
        """Base62."""
    ...
    def _is_const(self, vals: List[Any]) -> bool:
        """Constant check with lower threshold."""
    ...
    def _compress_array(self, arr: List[Any]) -> Union[List, Dict, str]:
        ...
        # Strings - use dictionary
        if all(isinstance(x, str) for x in arr):
            if self._is_const(arr):
                return {"c": self._c(arr[0]), "n": len(arr)}
    ...
    def compress(self, data: Any) -> Dict[str, Any]:
        ...
        if self.str_dict:
            r["s"] = {v: k for k, v in self.str_dict.items()}
        
        return r
    
    def _d(self, v: Any, km: Dict[str, str], sm: Dict[str, str]) -> Any:
        """Decompress value."""
        if isinstance(v, dict):
            if "r" in v:
                return sm.get(v["r"], "")
            elif "c" in v and "n" in v:
                return [self._d(v["c"], km, sm) for _ in range(v["n"])]
            elif "s" in v and "d" in v and "n" in v:
                return [v["s"] + i * v["d"] for i in range(v["n"])]
            elif "a" in v and v.get("a") == 1:
                return self._d_arr(v, km, sm)
            else:
                return {km.get(k, k): self._d(val, km, sm) for k, val in v.items()}
        elif isinstance(v, list):
            return [self._d(x, km, sm) for x in v]
        elif isinstance(v, str):
            if v.startswith("S"):
                return base64.b64decode(v[1:]).decode('utf-8')
            elif v.startswith("T"):
                return self._unpack_bools(v)
            elif v and v[0] in "UBHID":
                return self._unpack_nums(v)
        return v
    
    def _d_arr(self, ad: Dict, km: Dict[str, str], sm: Dict[str, str]) -> List[Dict]:
        """Decompress array."""
        sks = ad["k"]
        cols = ad["d"]
        ...
        for col in cols:
            if isinstance(col, dict):
                if "c" in col:
                    ecs.append([self._d(col["c"], km, sm)] * col["n"])
                elif "s" in col:
                    ecs.append([col["s"] + i * col["d"] for i in range(col["n"])])
            elif isinstance(col, str) and col and col[0] in "UBHIDT":
                ecs.append(self._d(col, km, sm))
            else:
                ecs.append(col)
        ...
        for i in range(nr):
            ...
            for j, sk in enumerate(sks):
                obj[km.get(sk, sk)] = self._d(ecs[j][i], km, sm)
            r.append(obj)
    ...
    def decompress(self, compressed: Dict[str, Any]) -> Any:
        ...
        return self._d(compressed["d"], rkm, sm)
