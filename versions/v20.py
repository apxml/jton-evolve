# EVOLVE-BLOCK-START
"""
JSON Compressor v20 - Conditional key map inclusion

Key improvements:
1. Keep all successful encodings from v8
2. Lower array columnar threshold to 1 (always use columnar for homogeneous arrays)
3. Optimize metadata representation with even shorter keys
"""
...
class JSONCompressor:
    ...
    def _is_sequential(self, numbers: List[Union[int, float]]) -> bool:
        ...
        if len(numbers) < 2:  # Lower threshold
            return False
    ...
    def _is_constant(self, values: List[Any]) -> bool:
        ...
        if len(values) < 1:  # Even single value can be "constant"
            return False
