# EVOLVE-BLOCK-START
"""
JSON Compressor v15 - List encoding for constant string values

Key improvements:
1. Detect constant string columns and use single-char encoding
2. Map common values like "premium", "free" to indices
"""
...
class JSONCompressor:
    ...
    def _encode_enum_column(self, values: List[Any]) -> Union[List, Dict]:
        """Encode column with limited unique values using enum mapping."""
        if not values or len(values) < 3:
            return values
        
        unique_vals = list(set(values))
        # Only encode if we have small number of unique values and they're strings
        if len(unique_vals) <= 4 and all(isinstance(v, str) for v in values):
            # Create mapping
            val_map = {v: i for i, v in enumerate(unique_vals)}
            encoded = [val_map[v] for v in values]
            return {"e": encoded, "v": unique_vals}
        return values
        ...
        if all(isinstance(item, dict) for item in arr):
            ...
            if all(set(item.keys()) == first_keys for item in arr):
                ...
                for orig_key, short_key in zip(keys, short_keys):
                    ...
                    # Try enum encoding for string columns with few unique values
                    elif all(isinstance(v, str) for v in col_values):
                        encoded = self._encode_enum_column(col_values)
                        columns.append(encoded if isinstance(encoded, dict) else [self._compress_value(v) for v in col_values])
                    else:
    ...
    def _decompress_value(self, value: Any, key_map: Dict[str, str]) -> Any:
        ...
        if isinstance(value, dict):
            ...
            if "e" in value and "v" in value:
                return [value["v"][i] for i in value["e"]]
            # Check for constant encoding
            elif "c" in value and "n" in value:
    ...
    def _decompress_array(self, arr_data: Dict, key_map: Dict[str, str]) -> List[Dict]:
        ...
        for col in columns:
            if isinstance(col, dict):
                if "e" in col and "v" in col:
                    # Enum-encoded column
                    expanded_columns.append([col["v"][i] for i in col["e"]])
                elif "c" in col:
                    # Constant column
