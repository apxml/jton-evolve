# EVOLVE-BLOCK-START
"""
JSON Compressor v28 - Multi-byte varint encoding

Key improvements:
1. Keep v26/v27 base
2. Add LEB128-style varint for large integers
3. Optimize array threshold (2+ items for packing)
4. More aggressive null/boolean packing
"""
...
class JSONCompressor:
    ...
    def _common_prefix(self, strings: List[str]) -> Tuple[str, List[str]]:
        ...
        if len(prefix) >= 2:
            ...
            if len(prefix) + sum(len(s) for s in suffixes) < sum(len(s) for s in strings):
                return prefix, suffixes
        return "", strings
    
    def _pack_nums(self, nums: List[Union[int, float]]) -> str:
        # Optimize for ID sequences (small positive ints)
    ...
    def _unpack_nums(self, s: str) -> List[Union[int, float]]:
        ...
        if t == 'U':
            ...
            return [s / 100 for s in struct.unpack(f'{len(d)//2}h', d)]
        ...
        if all(isinstance(x, dict) for x in arr):
            ...
            if all(set(x.keys()) == ks for x in arr):
                ...
                for k in keys:
                    ...
                    elif all(isinstance(v, bool) for v in cv) and len(cv) >= 2:
                        cols.append(self._pack_bools(cv))
                    elif all(isinstance(v, (int, float)) for v in cv) and len(cv) >= 2:
