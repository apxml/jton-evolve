"""
JSON Compressor - Maximum Optimization (v27)

Best performing compression algorithm from the AlphaEvolve experiment.
This version prioritizes maximum token reduction with non-human-readable output.

Key features:
- Ultra-compact single-character variable names
- Advanced prefix detection (min 2 chars)
- Multiple numeric packing formats (unsigned, signed, scaled floats)
- Boolean bit-packing (8 bools per byte)
- String common prefix extraction
...
- Token Savings: 62.2%
- Correctness: 100%
- Compression Time: 0.049ms
- Decompression Time: 0.029ms
- Score: 244.28
...
Usage:
    ...
    compressed = compress_json(data)  # Non-readable but highly compressed
...
class JSONCompressor:
    """Optimized prefix and binary compression."""
    ...
    def _common_prefix(self, strings: List[str]) -> Tuple[str, List[str]]:
        """Find common prefix (min 2 chars)."""
        ...
        if len(prefix) >= 2:  # Lower threshold
            suffixes = [s[len(prefix):] for s in strings]
            # Check if it saves space
            orig_len = sum(len(s) for s in strings)
            new_len = len(prefix) + sum(len(s) for s in suffixes)
            if new_len < orig_len:
    ...
    def _pack_nums(self, nums: List[Union[int, float]]) -> str:
        ...
        if all(isinstance(n, int) and 0 <= n <= 255 for n in nums):
            ...
            # Check if floats can be packed as scaled integers
            if all(isinstance(n, float) for n in nums):
                # Try 2 decimal places
                scaled = [int(n * 100) for n in nums]
                if all(abs(scaled[i]/100 - nums[i]) < 0.0001 for i in range(len(nums))):
                    if all(-32768 <= s <= 32767 for s in scaled):
                        return "F" + base64.b64encode(struct.pack(f'{len(scaled)}h', *scaled)).decode()
    ...
    def _unpack_nums(self, s: str) -> List[Union[int, float]]:
        ...
        if t == 'U':
            ...
            scaled = list(struct.unpack(f'{len(d)//2}h', d))
    ...
    def _d_arr(self, ad: Dict, km: Dict[str, str]) -> List[Dict]:
        ...
        for col in cols:
            ...
            elif isinstance(col, str) and col and col[0] in "UBVHIFDT":
